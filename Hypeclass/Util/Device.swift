...
struct Device {
    ...
    static var isNotch: Bool {
        return Double(UIApplication.shared.currentUIWindow()?.safeAreaInsets.bottom ?? -1) > 0

    }
    
    // MARK: 상태바 높이
    static var statusBarHeight: CGFloat {
        return UIApplication.shared.currentUIWindow()?.windowScene?.statusBarManager?.statusBarFrame.height ?? 0
    }
    ...
    static var topInset: CGFloat {
        return UIApplication.shared.currentUIWindow()?.safeAreaInsets.top ?? 0
    }
    
    // MARK: 디바이스의 아래쪽 여백 (Safe Area 아래쪽 여백)
    // ** 아래쪽 여백의 전체 높이 : bottomInset + tabBarHeight(존재하는 경우) **
    static var bottomInset: CGFloat {
        return UIApplication.shared.currentUIWindow()?.safeAreaInsets.bottom ?? 0
    }
}
